...
Как и в M1–M3, задача не добавляет новых commonTest-требований (чистка + структурная миграция, не новая бизнес-логика). Проверено: `SetupStartDestinationTest.kt` — единственный существующий тест, касающийся `IdentoNavHost`, — тестирует только чистую функцию `resolveStartDestination()`, а не сам `composable{}`-граф; нигде в кодовой базе нет прецедента юнит-теста, проверяющего фактическую регистрацию перехода между двумя экранами внутри `NavHost` (это потребовало бы Compose UI test harness, которого сейчас нет ни в одном commonTest). Поэтому §4 (вход в Settings) не получает отдельного автотеста навигации — корректность типа параметра `onNavigateToSettings: () -> Unit` проверяется компилятором, а фактическая регистрация перехода — гейтом `:androidApp:assembleDebug`/`:androidApp:lintDebug` и, при желании, ручным кликом. Введение нового test harness ради одного перехода было бы избыточно (YAGNI) и не соответствовало бы установленному в проекте стилю тестирования.
