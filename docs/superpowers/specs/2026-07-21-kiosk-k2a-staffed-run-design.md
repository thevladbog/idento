...
Все 5 шагов визуально переезжают на `PreflightShell`/`KioskInput`/`KioskButton` (тёмный kiosk-strict — иначе визард наполовину светлый shadcn, наполовину тёмный kiosk, что ломает целостность на реальном экране станции). «Как сейчас» ниже относится к **логике/API-вызовам**, не к визуалу — они переносятся без изменений.

1. **Подключение** — URL backend + health-check (логика как сейчас, визуал — новый).
2. **Вход** — пароль/staff-QR (логика как сейчас, визуал — новый).
