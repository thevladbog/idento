...
- **The old Login→Events→Checkin flow (`CheckinViewModel`, `AttendeeApiService.checkinAttendee` plain `PUT`) is not touched, not reused, and not extended.** Registration mode is built fresh against the idempotent batch-checkin contract (`POST /api/events/:event_id/checkins/batch`), per the design spec's explicit intent (§5.1 item 4) — the old plain-PUT endpoint has no idempotency and is being phased out.
  > **Approved exception (post-hoc note, added after the whole-branch review — see the summary doc's section (g), finding 2):** Task 4 changed `getAttendeeByCode`'s contract to return `null` instead of throwing on an empty match, which is a compile-forced change that also touched `CheckinViewModel.onCodeScanned`/`QRScannerViewModel.onQRScanned` (both call the same method). The reviewer confirmed the edit was minimal (only the `Error`-branch message text changed) and did not reuse, extend, or re-wire either old screen onto the new batch-checkin contract — the constraint above is about *not building Registration mode on top of the old flow*, which this compatibility fix did not do.
...
> **Post-hoc note (added after Task 6's implementation — see the summary doc's section (g), finding 4):** the "acceptable to construct a second instance" fallback above turned out to be a real risk, not just a stylistic wastefulness — neither `SqlDriverFactory.android.kt` nor `.ios.kt` configures WAL mode or a busy-timeout, so two separate connections to the same physical file would rely on SQLite's default rollback-journal locking, which is a real lock-contention risk once `SyncService` (Task 7) drives both queues' flush operations. Task 6 did NOT take this fallback: it refactored `SqlDelightOfflineDatabase`'s constructor to accept an injected `IdentoDatabase` instead, so exactly one instance is shared. Treat this fallback text as superseded — a future plan should require the shared-instance approach outright rather than presenting the second-connection path as an acceptable default.
