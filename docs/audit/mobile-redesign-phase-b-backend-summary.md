...
## (b) Новые объекты БД (Задача 1, миграция `000014_mobile_stations`)

**Переименована из `000013_mobile_stations` в `000014_mobile_stations`
при разрешении конфликта слияния с `main`:** пока эта ветка была открыта,
в `main` смержилась PR #26 ("Phase 1 Batch 1"), добавившая свою миграцию
`000013_tenant_lifecycle`. Обе миграции параллельно получили номер
`000013` — раннер миграций (`RunMigrations`, `pg_store.go`) определяет
версию по префиксу имени файла до первого `_`, поэтому при совпадении
номеров вторая (по алфавиту) миграция была бы молча пропущена как "уже
применённая". Ни одной строкой SQL эти две миграции не пересекаются
(`tenant_lifecycle` трогает только `tenants`; `mobile_stations` — свои 5
новых таблиц + `zone_access_rules`), поэтому конфликт чисто
нумерационный — решён переименованием файла в `000014_mobile_stations`
