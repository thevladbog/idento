...
| github.com/jackc/pgx/v5 | v5.7.6 | v5.10.0 | minor (**обязательно**) | 2 уязвимости: GHSA-9jj7-4m8r-rfcm / CVE-2026-33816 (CRITICAL, memory-safety, fixed 5.9.0) и GO-2026-5004 / GHSA-j88v-2chj-qfwx (SQL injection через путаницу плейсхолдеров с dollar-quoted строками, fixed 5.9.2). Обе достижимы по трассе govulncheck (`store.PGStore.GetAuditLog`) | api.osv.dev + govulncheck-backend.txt + go-outdated-backend.txt |
...
| golang.org/x/crypto (индирект. также в agent — не используется agent напрямую) | — | — | — | см. строку выше (общий для backend/agent module graph, но agent не импортирует crypto) | — |
