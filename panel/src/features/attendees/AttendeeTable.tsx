    ...
    <div data-testid="attendee-table" className="overflow-hidden rounded-[10px] border border-border bg-card">
      ...
      <ul className="flex flex-col divide-y divide-border">
        {rows.map((row) => {
          ...
          return (
            // The interactive role/handlers live on the inner <div>, not
            // this <li>: a11y audit (jsx-a11y/no-noninteractive-element-to-
            // interactive-role) — <li>'s implicit role is `listitem`, and
            // AT list-navigation (e.g. "list of N items") relies on that;
            // overwriting it with role="button" used to pull every row out
            // of list semantics entirely. The <li> stays a plain wrapper so
            // the <ul> is still announced as a real list, while the <div>
            // below keeps 100% of the existing click/keyboard/aria-label
            // behavior (RTL's getByRole matches by computed role, not tag,
            // so this doesn't change what AttendeeTable.test.tsx queries).
            <li key={row.id}>
              <div
              ...
              >
                ...
                {/* Fix (Codex, PR #65): previously exposed role=button (native
                    ...
                    bubbles to the row's onClick unchanged, preserving "click
