...
describe("golden ZPL matrix -- valign (2026-07-20 live-run request: top/middle/bottom on the native path, still dropped on raster)", () => {
  ...
  it("still DROPS valign on the raster path -- Cyrillic text with valign middle keeps the unadjusted ^FO (parity limitation, pinned)", async () => {
    const deps = makeDeterministicDeps();
    const elements: RawBadgeElement[] = [
    ...
    // call -- deliberately identical hex. The ^FO stays at 59,59: no +34
    // middle offset, because the raster branch returns before the valign
    // block. This is WHY the Properties pane only offers valign on
    // native-path elements until the raster-align work lands.
    expect(zpl).toBe(HEADER_300 + "^FO59,59\n^GFA,4,4,4,AA5FCB93\n^FS\n" + FOOTER);
